...
export class UserService {
  ...
  async verifyCredentials(
    ...
    //     return admin.auth().createCustomToken(user.id, payload);
    // }

    // Genera un token JWT para el usuario de todos sus datos
    async generateJWT(user: any): Promise<string> {
    ...
    // getUserByUsername
    async getUserByUsername(collectionName: string, username: string): Promise<any | null> {
        const db = admin.firestore();
        const collectionRef = db.collection(collectionName);
        const querySnapshot = await collectionRef
            .where('username', '==', username)
        ...
        if (!querySnapshot.empty) {
            // Si se encuentra un documento con el nombre de usuario, genera el token y devuelve todos los datos del usuario
            const userDoc = querySnapshot.docs[0].data();
            const userId = querySnapshot.docs[0].id;
            return { id: userId, ...userDoc };
        } else {
            // Si no se encuentra un documento con el nombre de usuario, devuelve null
            return null;
        }
    }
}
